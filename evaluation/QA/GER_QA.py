...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run German QA with specified adapters")
    parser.add_argument('--language_adapter_type', type=str, required=False,
                        help='Specify the type of language adapter (e.g., german_lora8)')
...
def load_and_prepare_dataset():
    ...
    # If no separate validation split is available, reuse the test split as validation.
    raw_datasets = dict(raw_datasets)
    if "val" not in raw_datasets:
        raw_datasets["val"] = raw_datasets["test"]
    return raw_datasets
...
def preprocess_validation_examples(examples):
    # Similar to the Swedish version’s validation preprocessing.
    questions = [q.strip() for q in examples["question"]]
    inputs = tokenizer(
        questions,
        examples["context"],
...
def compute_metrics(start_logits, end_logits, features, examples):
    ...
    for example in tqdm(examples, desc="Computing Metrics"):
        ...
        if answers:
            ...
            # Convert the id to a string for both predictions and references
...
def train_and_evaluate_kfold(args, train_dataset, validation_dataset, test_dataset, raw_datasets):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(train_dataset)):
        print(f"\n===== Fold {fold + 1} =====")
        # For training we use the training-preprocessed examples
        train_fold = train_dataset.select(train_index)
        # Although we split the train set, the evaluation will be done on the held-out validation set.
        ...
        if args.language_adapter_type is not None:
            model.load_adapter("rominaoji/" + args.language_adapter_type)

        task_adapter = args.task_adapter_type
        model.add_adapter("qa", config=task_adapter)
        ...
        # Evaluate on the held-out validation set (using the preprocessed validation split)
        val_predictions = trainer.predict(validation_dataset)
        val_start_logits, val_end_logits = val_predictions.predictions
        val_metrics = compute_metrics(val_start_logits, val_end_logits, validation_dataset, raw_datasets["val"])
        ...
        test_metrics = compute_metrics(test_start_logits, test_end_logits, test_dataset, raw_datasets["test"])
...
def main():
    ...
    print(f"German QA task adapter: {args.task_adapter_type} | language adapter: {args.language_adapter_type}")

    # Global parameters used by multiple functions.
    ...
    # Preprocess the training examples (for adapter training)
    train_dataset = raw_datasets["train"].map(
        preprocess_training_examples,
        batched=True,
        remove_columns=raw_datasets["train"].column_names,
    )
    # For evaluation we use the validation-style preprocessing.
    ...
    all_val_metrics, all_test_metrics = train_and_evaluate_kfold(
        args, train_dataset, validation_dataset, test_dataset, raw_datasets
    )

    mean_val_exact_match, std_val_exact_match, mean_val_f1, std_val_f1 = calculate_mean_std(all_val_metrics)
    print(f"\nK-Fold Validation Exact Match: {mean_val_exact_match:.2f} ± {std_val_exact_match:.2f}")
    ...
    print(f"\nK-Fold Test Exact Match: {mean_test_exact_match:.2f} ± {std_test_exact_match:.2f}")
    print(f"K-Fold Test F1 Score: {mean_test_f1:.2f} ± {std_test_f1:.2f}")

    print(f"\nGerman QA  task adapter: {args.task_adapter_type} | language adapter: {args.language_adapter_type}")
