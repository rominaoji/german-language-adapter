...
def load_and_prepare_dataset():
    dataset = load_dataset("MIM-GOLD-NER", name="mim-gold-ner", trust_remote_code=True)

    label_names = dataset["train"].features["conll_ner_tags"].feature.names
...
def tokenize_adjust_labels(all_samples_per_split, tokenizer, label_names):
    tokenized_samples = tokenizer.batch_encode_plus(all_samples_per_split["tokens"], is_split_into_words=True)
    total_adjusted_labels = []
    for k in range(len(tokenized_samples["input_ids"])):
        prev_wid = -1
        word_ids_list = tokenized_samples.word_ids(batch_index=k)
        existing_label_ids = all_samples_per_split["conll_ner_tags"][k]
        i = -1
        adjusted_label_ids = []
...
def train_and_evaluate_kfold(dataset, tokenizer, label_names, id_2_label, label_2_id):
    tokenized_dataset = dataset.map(lambda x: tokenize_adjust_labels(x, tokenizer, label_names), batched=True)
...
def main():
    ...
    all_val_metrics, all_test_metrics = train_and_evaluate_kfold(dataset, tokenizer, label_names, id_2_label, label_2_id)
